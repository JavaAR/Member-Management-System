...
layui.config({
    ...
    form.on("submit(changeUser)", function (data) {
       ...
       /* date.setDate(date.getDate());*/
        ...
        /*var className =  $('#showUserClass').val();*/
        //发送请求
        $.ajax({
            url:getRootPath()+'/api/updateStudentInfo',
            headers: {
                "token": token
            },
            type: 'post',
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.success == "true") {
                ...
                var userLever = 'dd[lay-value=' + userlever + ']';//选中课程强度
                $('#selectuselever').siblings("div.layui-form-select").find('dl').find(userLever).click();
